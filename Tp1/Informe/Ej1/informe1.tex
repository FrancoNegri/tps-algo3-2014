...
El siguiente es un pseudo-c\'odigo de nuestro algoritmo.\\
 ...
 \caption{ \\Saltos( saltoMax : natural, puente : arreglo(1's y 0's), distancias: arreglo(naturales) , cantidadTablones : natural)}
 		...
 		\STATE{\quad mientras posActual < cantidadTablones} \;\; O(n)
 			\STATE{\quad\quad SI puente.indice(posActual) es 0} \;\; 
 				\STATE{\quad\quad\quad posActual \leftarrow posActual + 1} \;\; O(1)\\
 			\STATE{\quad\quad SI NO}\\
 				\STATE{\quad\quad\quad SI posActual - saltoMax < 0} \;\; 
 					\STATE{\quad\quad\quad\quad distancias.indice(posActual) \leftarrow 1}\;\; O(1) \\
 					\STATE{\quad\quad\quad\quad posActual \leftarrow posActual + 1} \;\; O(1)\\
 				\STATE{\quad\quad\quad SI NO}
 					\STATE{\quad\quad\quad\quad minimoSalto \leftarrow BUSCOMINIMO(posActual  - saltoMax, posActual, puente)} \;\; O(saltoMax) \\
 					\STATE{\quad\quad\quad distancias.indice(posActual) \leftarrow minimoSalto + 1}\;\; O(1) \\
 					\STATE{\quad\quad\quad posActual \leftarrow posActual + 1} \;\;O(1) \\
 \\

...
El paso "Miro que peso agrega el nodo $i$ quitandolo del conjunto asignado y poniendolo en los demas" itera sobre todos los elementos de los conjuntos, al menos voy a iterar n veces, luego la complejidad es $\mathcal{O}(n)$.

Dada nuestra implementacion de conjunto con listas simplemente enlazadas, encontrar un elemento y borrarlo cuesta $\mathcal{O}(n^2)$ por esta razón modificar la solución previa tendrá esta complejidad. Utilizando listas doblemente enlazadas esto se hubiera podido reducir a $\mathcal{O}(n)$ pero por falta de tiempo no se implementó.
